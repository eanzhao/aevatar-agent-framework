...
public class MemoryManagementTests
{
    ...
    public async Task Memory_WithCancellation_ShouldThrow()
    {
        ...
        await Should.ThrowAsync<OperationCanceledException>(
            async () => await memory.AddMessageAsync("user", "Test", cts.Token));
        
        await Should.ThrowAsync<OperationCanceledException>(
            async () => await memory.GetConversationHistoryAsync(cancellationToken: cts.Token));
        
        await Should.ThrowAsync<OperationCanceledException>(
            async () => await memory.ClearHistoryAsync(cts.Token));
        
        await Should.ThrowAsync<OperationCanceledException>(
            async () => await memory.SearchAsync("test", cancellationToken: cts.Token));
    }
