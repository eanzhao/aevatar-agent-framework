namespace Aevatar.Agents.AI.Abstractions.Tests.Memory;
...
{
    private readonly List<AevatarConversationEntry> _history = new();
    private readonly List<(string query, IReadOnlyList<string> results)> _searchHistory = new();
    
    public IReadOnlyList<AevatarConversationEntry> History => _history;
    ...
    public Task AddMessageAsync(string role, string content, CancellationToken cancellationToken = default)
    {
        ...
        _history.Add(new AevatarConversationEntry
        {
            Role = role,
            Content = content
        });
