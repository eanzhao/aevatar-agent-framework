...
public class CodingGAgentTests
{
    ...
    {
        // Arrange & Act
        var agent = new CodingGAgent(_mockServiceProvider.Object, _mockFactory.Object, _mockSerializer.Object);
        ...
        Assert.Empty(state.ValidationHistory);
    }
    ...
    public async Task RegisterEventHandlersAsync_SubscribesToEvents()
    {
        ...
        _mockStream.Verify(s => s.SubscribeAsync<CodeValidationEvent>(
                It.IsAny<Func<CodeValidationEvent, Task>>(),
    ...
    [Fact]
    public async Task ApplyEventAsync_CodeValidationEvent_UpdatesState()
    {
        // Arrange
        var agent = new CodingGAgent(_mockServiceProvider.Object, _mockFactory.Object, _mockSerializer.Object);
        var codeEvent = new CodeValidationEvent
        {
            Code = "var x = 42;",
            Result = true
        };
        ...
        // Assert
        var state = agent.GetState();
        Assert.Equal($"ValidationResult: {codeEvent.Result}", state.ValidationHistory);
    ...
    public async Task ApplyEventAsync_ConfigEvent_UpdatesState()
    {
        ...
        // Assert
        var state = agent.GetState();
        Assert.Equal(configEvent.ConfigValue, state.ValidationHistory);
        Assert.Equal(evt.Version, state.CurrentVersion);
    }
