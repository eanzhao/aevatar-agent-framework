    ...
    where TState : class, IMessage, new()
{
    ...
    {
        var keywords = new[] { "analyze", "compare", "evaluate", "assess", "explore", 
                               "分析", "比较", "评估", "探索" };
    ...
    {
        var keywords = new[] { "step by step", "explain", "how", "why", "reasoning",
                               "步骤", "解释", "为什么", "怎么", "推理" };
        return keywords.Any(k => message.ToLower().Contains(k));
    }
    ...
    {
        var keywords = new[] { "calculate", "search", "query", "fetch", "execute",
                               "计算", "搜索", "查询", "获取", "执行" };
        return keywords.Any(k => message.ToLower().Contains(k));
    }
    ...
    private class LLMProviderAdapter : IAevatarLLMProvider
    {
        ...
        public LLMProviderAdapter(ILLMProvider innerProvider)
        {
