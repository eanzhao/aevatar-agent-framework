...
namespace Aevatar.Agents.AI.Core.Tools.CustomTools;
...
public class HttpRequestTool : AevatarToolBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (HttpRequestException ex)
        {
            ...
            return new { success = false, error = $"HTTP request failed: {ex.Message}" };
        }
        catch (TaskCanceledException)
        {
            logger?.LogWarning("HTTP request timeout");
            return new { success = false, error = "Request timeout" };
        }
        catch (Exception ex)
        {
