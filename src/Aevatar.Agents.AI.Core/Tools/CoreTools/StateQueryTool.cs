...
public class StateQueryTool : AevatarToolBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!validation.IsValid)
        {
            ...
            return Task.FromResult<object?>(new { success = false, errors = validation.Errors });
        }
        
        if (context.GetStateCallback == null)
        {
            logger?.LogWarning("GetStateCallback not provided, cannot query state");
            return Task.FromResult<object?>(new { success = false, error = "State querying not available" });
        }
        ...
        try
        {
            ...
            if (state == null)
            {
                return Task.FromResult<object?>(new { success = false, error = "State is null" });
            }
