...
namespace Aevatar.AuthServer.New.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateScopesAsync()
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Aevatar") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                ...
                Resources = { "Aevatar" } // Use "Aevatar" scope like old project
            });
        }
        
        // Create BusinessServer scope if it doesn't exist
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            "Aevatar", // Use "Aevatar" scope like old project
            "New",
            "BusinessServer"
        };
        ...
        var consoleAndAngularClientId = configurationSection["New_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["New_App:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["New_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["New_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // AuthServer Client (for self-authentication)
        var authServerClientId = configurationSection["New_AuthServer:ClientId"];
        if (!authServerClientId.IsNullOrWhiteSpace())
        {
            var authServerRootUrl = configurationSection["New_AuthServer:RootUrl"]?.TrimEnd('/');
            ...
            await CreateApplicationAsync(
                ...
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "AuthServer Application",
                secret: null,
                grantTypes: new List<string>
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode,
        ...
        // BusinessServer Client (for API authentication and Web UI)
